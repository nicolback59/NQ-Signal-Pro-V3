...
// Render's health check fires immediately after the process launches. All
// migration IIFEs are synchronous and block the event loop for several seconds,
// keeping the port closed and causing "connection refused". Calling app.listen()
// here binds the OS-level socket immediately; the callback fires only after all
// synchronous code (migrations + routes) has completed, so every variable it
// references is guaranteed to be defined when it runs.
