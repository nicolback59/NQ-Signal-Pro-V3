...
export type ExpirationReason =
  | 'EXPIRED_MARKET_HOURS'
  | 'EXPIRED_MARKET_CLOSE'
  | 'EXPIRED_WEEKEND_CLOSE'
  | 'EXPIRED_MAX_HOLD'
  | 'EXPIRED_STUCK_TRADE';
...
export interface OHLCV {
  timestamp: number | string | Date; // epoch ms, ISO string, or Date
  open: number;
  high: number;
  low: number;
  close: number;
...
export interface HTFBiasResult {
  score15m: number;  // -1 | 0 | 1
  score30m: number;
  score45m: number;
  score1h: number;
  totalScore: number; // -4 to +4
  bullish: boolean;   // totalScore >= 2
  bearish: boolean;   // totalScore <= -2
}
...
export interface ESIndicators {
  ...
  htfBullish: boolean;
  htfBearish: boolean;
  makingHHHL: boolean;  // higher highs and higher lows
  makingLHLL: boolean;  // lower highs and lower lows
  momentumBullish: boolean;
  momentumBearish: boolean;
  htfFlippedBeforeMNQ: boolean; // ES leadership signal
  ema9: number;
  ema21: number;
  vwap: number;
}
...
export interface ESConfirmResult {
  confirmed: boolean;
  direction: Direction;
  confidenceBonus: number;    // +5 for ES leadership, -10 for MNQ moved without ES
...
export interface SignalScore {
  ...
  adxScore: number;      // 0-5
  esScore: number;       // 0-20
  breadthBonus: number;  // +5 to +10
  esLeadershipBonus: number; // +5 or -10
}
...
export interface EvaluationParams {
  instrument: Instrument;
  bars: OHLCV[];          // 1m or 5m bars, at least 50
  bars5m?: OHLCV[];       // 5m bars for HTF aggregation (if bars are 1m)
  esBars?: OHLCV[];       // ES 5m bars (required for MNQ)
  es5mBars?: OHLCV[];     // ES 5m bars for HTF
  nqBars?: OHLCV[];       // NQ bars for market breadth
  timestamp: Date;
...
export interface LearningWeights {
  mnqMinScore?: number;    // default 70, AI can adjust 65-85
  mgcMinScore?: number;    // default 70, AI can adjust 65-80
  trendWeight?: number;    // default 25, AI can adjust 20-30
  htfWeight?: number;      // default 20, AI can adjust 15-25
  vwapWeight?: number;     // default 15, AI can adjust 10-20
  momentumWeight?: number; // default 10, AI can adjust 7-15
  atrWeight?: number;      // default 5, AI can adjust 3-8
  adxWeight?: number;      // default 5, AI can adjust 3-8
  esWeight?: number;       // default 20, AI can adjust 15-25
  // AI learning CANNOT modify: risk management, stop methodology, position sizing, core filters
}

// Internal scoring params
...
export const DEFAULT_LEARNING_WEIGHTS: Required<LearningWeights> = {
  mnqMinScore: 70,
  mgcMinScore: 70,
  trendWeight: 25,
  htfWeight: 20,
...
const ATR_MIN: Record<Instrument, number> = {
  MNQ: 18,
  MGC: 2.0,
};
 ...
 */
function getEtWeekday(ts: Date): number {
  const parts = new Intl.DateTimeFormat('en-US', {
    timeZone: 'America/New_York',
    weekday: 'short',
  }).formatToParts(ts);
 ...
 */
function getSessionName(hhmm: number): string {
  if (hhmm >= 930 && hhmm < 1130) return 'NY Open';
  if (hhmm >= 200 && hhmm < 730) return 'London';
  if (hhmm >= 730 && hhmm < 930) return 'London/NY Overlap';
  if (hhmm >= 1130 && hhmm < 1300) return 'Midday';
  if (hhmm >= 1300 && hhmm < 1600) return 'Afternoon';
 ...
 */
export function isMarketOpen(ts: Date): { open: boolean; reason?: string } {
 ...
 */
export function calcAtr(bars: OHLCV[], period = 14): (number | null)[] {
...
export function calcAdx(
  ...
  const diPlusArr: (number | null)[] = rDmPlus.map((v, i) =>
    atrSmooth[i] != null && atrSmooth[i]! > 0 ? (v! / atrSmooth[i]!) * 100 : null,
  );
  const diMinusArr: (number | null)[] = rDmMinus.map((v, i) =>
    atrSmooth[i] != null && atrSmooth[i]! > 0 ? (v! / atrSmooth[i]!) * 100 : null,
  );

  const dx: (number | null)[] = diPlusArr.map((v, i) => {
...
function aggregateBars(bars: OHLCV[], factor: number): OHLCV[] {
  const out: OHLCV[] = [];
...
export function computeIndicators(bars: OHLCV[]): Indicators {
  ...
  const atrVal = (atrArr[n] as number | null) ?? 0;
  const adxVal = (adxResult.adx[n] as number | null) ?? 0;
  const rsiVal = (rsiArr[n] as number | null) ?? 50;
 ...
 *    0 otherwise (neutral)
 */
export function computeHTFBias(bars5m: OHLCV[]): HTFBiasResult {
  function scoreTF(tfBars: OHLCV[]): number {
    if (tfBars.length < 21) return 0;
 ...
 */
export function computeESIndicators(esBars: OHLCV[], es5m?: OHLCV[]): ESIndicators {
 ...
 */
export function checkESConfirmation(
  esInd: ESIndicators,
  direction: Direction,
  htfBias: HTFBiasResult,
): ESConfirmResult {
  ...
  if (!confirmed) {
    ...
    if (isLong) {
      if (!esInd.makingHHHL) missing.push('NO_HHHLS');
      if (!esInd.momentumBullish) missing.push('ES_MOMENTUM_NOT_BULL');
    } else {
      if (!esInd.makingLHLL) missing.push('NO_LHLLS');
      if (!esInd.momentumBearish) missing.push('ES_MOMENTUM_NOT_BEAR');
    }
    if (missing.length > 0) {
      return {
        confirmed: false,
 ...
 */
export function scoreMNQ(params: ScoreParams): SignalScore {
  ...
  let trendRaw = 0;
  if (isLong) {
    if (ind.ema9AboveEma21 && ind.aboveVwap) trendRaw = 1.0;
    else if (ind.ema9AboveEma21 || ind.aboveVwap) trendRaw = 0.6;
    else trendRaw = 0.32;
  } else {
    if (!ind.ema9AboveEma21 && !ind.aboveVwap) trendRaw = 1.0;
    else if (!ind.ema9AboveEma21 || !ind.aboveVwap) trendRaw = 0.6;
  ...
  let htfRatio = 0;
  if (absHtf >= 3) htfRatio = 1.0;
  else if (absHtf === 2) htfRatio = 0.75;
  else if (absHtf === 1) htfRatio = 0.4;
  else htfRatio = 0;
  ...
  let vwapScore = 0;
  if (isLong) {
    if (ind.aboveVwap && ind.ema9AboveEma21) vwapScore = vwapMax;
    else if (ind.aboveVwap) vwapScore = Math.round(vwapMax * 0.53);
    else vwapScore = 0;
  } else {
    if (!ind.aboveVwap && !ind.ema9AboveEma21) vwapScore = vwapMax;
    else if (!ind.aboveVwap) vwapScore = Math.round(vwapMax * 0.53);
  ...
  const rsi = ind.rsi;
  if (isLong) {
    if (rsi >= 50 && rsi <= 65 && ind.macdHist > 0) momScore = momMax;
    else if ((rsi >= 50 && rsi < 70) || ind.macdHist > 0) momScore = Math.round(momMax * 0.5);
    else if (rsi >= 45) momScore = Math.round(momMax * 0.2);
  } else {
    if (rsi <= 50 && rsi >= 35 && ind.macdHist < 0) momScore = momMax;
    else if ((rsi <= 50 && rsi > 30) || ind.macdHist < 0) momScore = Math.round(momMax * 0.5);
    else if (rsi <= 55) momScore = Math.round(momMax * 0.2);
  }
  ...
  let atrScore = 0;
  if (ind.atr >= atrMin * 1.5) atrScore = atrMax;
  else if (ind.atr >= atrMin * 1.15) atrScore = Math.round(atrMax * 0.6);
  else if (ind.atr >= atrMin) atrScore = Math.round(atrMax * 0.2);

  // ── ADX score (0–5) ─────────────────────────────────────────────────────
  const adxMax = W.adxWeight;
  let adxScore = 0;
  if (ind.adx > 25) adxScore = adxMax;
  else if (ind.adx > 20) adxScore = Math.round(adxMax * 0.8);
  else if (ind.adx > 16) adxScore = Math.round(adxMax * 0.4);

  // ── ES Confirmation score (0–20) ────────────────────────────────────────
  const esMax = W.esWeight;
  let esScore = 0;
  if (esConfirm != null) {
    if (esConfirm.confirmed) esScore = esMax;
    else esScore = Math.round(esMax * 0.5);
  } else {
  ...
  if (esConfirm != null) {
    if (esConfirm.esLeadership) esLeadershipBonus = 5;
    else esLeadershipBonus = -10;  // ES confirmed but lagged — MNQ moved without ES leading
  }
  ...
  const total = Math.min(
    100,
    trendScore + htfScore + vwapScore + momScore + atrScore + adxScore + esScore +
 ...
 */
export function scoreMGC(params: ScoreParams): SignalScore {
  const { direction, ind, htf, esConfirm, nqAligned, weights } = params;
  const isLong = direction === 'LONG';
  const W = weights;
 ...
 *   RR = 20 / stopDistance
 */
export function computeTargetsMNQ(
  entry: number,
  direction: Direction,
  atr: number,
  recentSwing: number,
 ...
 *   RR = 1.5
 */
export function computeTargetsMGC(
  entry: number,
  direction: Direction,
  atr: number,
  recentSwing: number,
 ...
 */
export function gradeSignal(score: number): Grade {
 ...
 */
export function shouldNotify(grade: Grade): boolean {
 ...
 */
function isNQAligned(nqBars: OHLCV[], direction: Direction): boolean {
  if (nqBars.length < 22) return false;
  ...
  if (direction === 'LONG') return e9 > e21 && close > vwapVal;
...
function recentSwingHigh(bars: OHLCV[], lookback = 10): number {
  const slice = bars.slice(-lookback - 1, -1);
...
export function evaluate(params: EvaluationParams): EvaluationResult {
  ...
  const weights: Required<LearningWeights> = {
  ...
  const direction: Direction = longTrend ? 'LONG' : 'SHORT';
  const isLong = direction === 'LONG';
  ...
  let esConfirm: ESConfirmResult | null = null;
  if (instrument === 'MNQ') {
  ...
  const signalScoreObj =
    instrument === 'MNQ' ? scoreMNQ(scoreParams) : scoreMGC(scoreParams);
  ...
  const swingRef =
    direction === 'LONG' ? recentSwingLow(bars, 10) : recentSwingHigh(bars, 10);
  ...
  const htfBiasLabel: 'BULL ▲' | 'BEAR ▼' | 'NEUTRAL' =
    htf.bullish ? 'BULL ▲' : htf.bearish ? 'BEAR ▼' : 'NEUTRAL';

  const ticker: 'MNQ1!' | 'MGC1!' = instrument === 'MNQ' ? 'MNQ1!' : 'MGC1!';
